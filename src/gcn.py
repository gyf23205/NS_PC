...
class GraphConvNet(nn.Module):
    def __init__(self, n_embed_channel, size_kernal, dim_observe, n_rel, n_head=8):
        super().__init__()
        self.n_rel = n_rel
        self.dim_map = dim_observe
        ...
        self.out = nn.Linear(self.dim_embed, dim_observe**2)


    def forward(self, x, neighbors):
        '''
        x.shape = (1, dim_map, dim_map)
        ...
        return F.softmax(torch.squeeze(self.out(x)), dim=0)
